...
- Both is conveniently done by using the functions `ur.df()` for computation of the ADF test and `ur.ers()` for conducting the DF-GLS test.
- We employ models that include a drift term. (If `type` argument is set to "none" neither an intercept nor a trend is included in the test regression. If it is set to "drift" an intercept is added and if it is set to "trend" both an intercept and a trend is added.)
- We set the maximum lag order to  5
and use the AIC for selection of the optimal lag length.


```{r testurcafun, echo = FALSE, eval = FALSE}
library(fpp3)
set.seed(123)
tbl1 <- tibble(
 ...
 as_tsibble(tbl1, index = date) %>%autoplot()
library(urca)
...
r2 <- ur.ers(tbl1$value,
             model = "constant", 
             lag.max = 6)
summary(r2)

r3 <- ur.ers(tbl1$value,
             model = "trend", 
             lag.max = 6)
summary(r3)
...
tbl2 <- tibble(
  date = as.Date("2017-01-01") + 0:98,
  value = Raotbl3$lc
) 
lc.df <- ur.df(y=lc, lags=3, type='trend')
summary(lc.df)

as_tsibble(tbl2, index = date) %>%autoplot()
...
- Note that the function `ur.df()` reports two test statistics when there is a drift in the ADF regression. 
...
#### Estimation of Cointegrating coefficient using Dynamic OLS Estimator (DOLS)
...
-There are many other alternative estimators of the cointegrating coefficient, such as **dynamic OLS (DOLS) estimator** (Stock and Watson, 1993).
