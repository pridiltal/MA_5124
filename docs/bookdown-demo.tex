...
\tightlist
\item
  The \textbf{sample cross-correlation} coefiicient of \(X_i\) and \(X_j\) at lag \(k\) is given by
...
\KeywordTok{MTSplot}\NormalTok{(ret)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ccm}\NormalTok{(ret, }\DataTypeTok{lags =} \DecValTok{25}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
