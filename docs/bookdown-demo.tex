...
\item
  ...
  \tightlist
  \item
    \(Y_t\) is serially uncorrelated, but dependent, (b) the dependence of \(Y_t\) can be described by a simple quadratic function of its lagged values.
  \end{enumerate}
\item
...
\item
  Then, it yeilds the following \(ARMA(p,q)-GARCH(m,s)\) model for \((X_t, \sigma_t):\)
\end{itemize}
...
\item
  The second equation can be replaced by other volatility models so that different spects of volatilities can be characterized.
\end{itemize}
...
\tightlist
\item
  Let's apply the modeling procedure to build an \(ARMA(1,1)-GARCH(1,1)\) model for the daily returns, \(Y_t,\) of adjusted closing prices of the S\&P500 index
\end{itemize}

\[X_t = \mu + \phi X_{t-1}+\theta Y_{t-1},\]
\[Y_t= \sigma_t\epsilon_t,\]
